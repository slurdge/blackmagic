...
if __name__ == "__main__":
	...
	for dfu in devs:
		handle = dfu[0].open()
		man = handle.getString(dfu[0].iManufacturer, 40)
		product = handle.getString(dfu[0].iProduct, 40)
		print("Device %s: ID %04x:%04x %s - %s" % (dfu[0].filename,
			dfu[0].idVendor, dfu[0].idProduct,
                        man.decode('utf-8'), product.decode('utf-8')))
